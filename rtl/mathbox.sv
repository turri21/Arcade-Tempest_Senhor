module mathBox
  ...
  always @(posedge clk) begin
    ...
    logic [7:0] status;
    logic [7:0] dataOut_reg;    // registered read  (Battlezone / Red Baron - HW-proven, unchanged)
    logic [7:0] dataOut_comb;   // combinational read (Tempest - polls status, must not lag)
    ...
    always_ff @(posedge clk) begin
      if (clk_en) begin
		  if (mod_tempest) begin
			  // Tempest maps the mathbox at $6040/$6060/$6070; tempest.vhd feeds
			  // the low byte of the CPU address as `addr` on reads.
			  if(addr == 8'h40) dataOut_reg <=status;        // $6040 status_r
			  else if(addr == 8'h60) dataOut_reg <=mbOutQ[7:0];   // $6060 lo_r
			  else if(addr == 8'h70) dataOut_reg <=mbOutQ[15:8];  // $6070 hi_r
			  else dataOut_reg <=8'hFF;
		  end
		  else if (mod_redbaron) begin
			  if(addr == 8'h00) dataOut_reg <=status;
			  else if(addr == 8'h04) dataOut_reg <=mbOutQ[7:0];
			  else if(addr == 8'h06) dataOut_reg <=mbOutQ[15:8];
			  else dataOut_reg <=8'hFF;
		  end
		  else begin
			  if(addr == 8'h00) dataOut_reg <=status;
			  else if(addr == 8'h10) dataOut_reg <=mbOutQ[7:0];
			  else if(addr == 8'h18) dataOut_reg <=mbOutQ[15:8];
			  else dataOut_reg <=8'hFF;
		  end
		end
    end

    // Tempest reads the math box COMBINATIONALLY.  Tempest polls status with BIT $6040/BMI;
    // a registered read returns the previous bus cycle's value, so the poll keeps reading the
    // clobbered $FF and the CPU hangs (-> $53 overflow -> IRQ reset/BRK -> crash). Reading
    // combinationally lets the poll release the instant the FSM is IDLE (status=$00, matching
    // MAME mathbox.cpp status_r "always done").  Battlezone/Red Baron keep the registered read.
    always_comb begin
      if      (addr == 8'h40) dataOut_comb = status;        // $6040 status_r
      else if (addr == 8'h60) dataOut_comb = mbOutQ[7:0];   // $6060 lo_r
      else if (addr == 8'h70) dataOut_comb = mbOutQ[15:8];  // $6070 hi_r
      else                    dataOut_comb = 8'hFF;
    end
    assign dataOut = mod_tempest ? dataOut_comb : dataOut_reg;
